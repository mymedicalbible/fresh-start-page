...
export function TestsOrderedPage () {
  ...
  async function saveTests () {
    ...
    setBusy(true)
    const { error: e } = await supabase.from('tests_ordered').insert(
      valid.map((t) => ({
        user_id: user!.id,
        test_date: formDate,
  ...
  async function loadDocs (testId: string) {
    ...
    const { data, error: e } = await supabase.storage.from('visit-docs').list(folder, { limit: 50 })
    if (e) return
    const signed = await Promise.all((data ?? []).map(async (f) => {
      const { data: sd } = await supabase.storage.from('visit-docs')
  ...
  async function uploadDoc (testId: string, file: File) {
    ...
    try {
      ...
      const { error: e } = await supabase.storage.from('visit-docs')
      ...
      {showForm && (
        <div className="card">
          ...
          <button type="button" className="btn btn-primary btn-block"
            onClick={saveTests} disabled={busy}>Save orders</button>
        </div>
      )}
