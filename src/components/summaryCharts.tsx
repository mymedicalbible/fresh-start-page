...
export function PainSummaryChart ({ data, title = 'Pain intensity' }: { data: PainChartPoint[]; title?: string }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: 'var(--mint-ink)' }}>{title}</div>
      ...
      <div className="charts-wrap" style={{ height: 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 6, right: 8, bottom: 4, left: -16 }}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
            <XAxis dataKey="label" tick={{ fontSize: 10 }} stroke="var(--muted)" />
            <YAxis domain={[0, 10]} ticks={[0, 3, 6, 9, 10]} tickFormatter={(v) => `${v}`} tick={{ fontSize: 10 }} stroke="var(--muted)" width={28} />
