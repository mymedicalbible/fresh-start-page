...
| `npm run export:txt` | One text file: all important source + SQL (see below) |
...
To generate a **single** dump of essentially all `.ts`, `.tsx`, `.sql`, configs, docs, workflows, etc. (excluding `node_modules`, `dist`, `exports`):

```bash
npm run export:txt
```

**Output (overwritten each run):** `exports/ALL_CODE_AND_SQL.txt`
